...
class Busyness_Footer_Widgets {
	 ...
	 * @var int
	 */
	protected $theme_prefix = 'busyness';
	...
	function init() {
		...
		if ( $this->active_widgets > 0 ) {
			...
			add_action( 'busyness_action_before_footer', array( $this, 'add_footer_widgets' ), 20 );
		}
	...
	function footer_widgets_init() {
		...
		for ( $i = 1; $i <= $this->max_widgets; $i++ ) {
			register_sidebar( array(
				'name'          => sprintf( __( 'Footer Widget %d', 'busyness' ), $i ),
...
// Initialize.
$busyness_footer_widgets = new Busyness_Footer_Widgets();
