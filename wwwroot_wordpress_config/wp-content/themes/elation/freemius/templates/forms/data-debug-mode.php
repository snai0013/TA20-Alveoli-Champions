...
$message_above_input_field = (!$has_last_license_user_id) ? fs_text_inline('Please enter the license key to enable the debug mode:', 'submit-developer-license-key-message', $slug) : sprintf(fs_text_inline('To enter the debug mode, please enter the secret key of the license owner (UserID = %d), which you can find in your "My Profile" section of your User Dashboard:', 'submit-addon-developer-key-message', $slug), $last_license_user_id);
...
$license_or_user_key_text = (!$has_last_license_user_id) ? fs_text_inline('License key', 'license-key', $slug) : fs_text_inline('User key', 'user-key', $slug);
...
?>' );
                ...
                error     : function () {
                    showError( <?php
echo json_encode(fs_text_inline('An unknown error has occurred.', 'unknown-error', $slug));
?> );
                    resetButtons();
                }
            });
